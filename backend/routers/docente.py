...
def criar_vaga(req: VagaRequest, docente=Depends(get_current_docente)):
    ...
    try:
        with conn.cursor() as cursor:
            # Chama a procedure publicar_vaga do banco de dados
            cursor.execute("CALL publicar_vaga(%s, %s, %s, %s, %s)", 
                           (req.titulo, req.descricao, req.data_limite, req.id_empresa, req.id_tipo_vaga))
            
            # Como a procedure não preenche a quantidade, se a quantidade for > 1, fazemos um update rápido
...
def listar_minhas_vagas(docente=Depends(get_current_docente)):
    # Simulação: Como as vagas não têm id_usuario (são atreladas à empresa no BD atual),
    # vamos retornar todas as vagas para o docente poder gerenciar os alunos.
    # Em um banco real completo, haveria a coluna "id_responsavel" na tabela vagas.
    conn = get_db_connection()
    try:
        with conn.cursor(cursor_factory=RealDictCursor) as cursor:
            cursor.execute("""
                SELECT v.id_vaga, v.titulo, e.nome_empresa, v.quantidade_vagas, v.data_limite, v.status
                FROM vagas v
